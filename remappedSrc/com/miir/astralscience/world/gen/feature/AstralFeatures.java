...
public abstract class AstralFeatures {
    ...
    private static final StructureFeature<DefaultFeatureConfig> CASCADIUM_SKELETON = new CascadiumSkeletonFeature();
    private static final ConfiguredStructureFeature<?, ?> CASCADIUM_SKELETON_CONFIGURED = CASCADIUM_SKELETON.configure(DefaultFeatureConfig.DEFAULT);
    ...
    public static void register() {
        ...
        FabricStructureBuilder.create(AstralScience.id("cascadium_skeleton"), CASCADIUM_SKELETON)
                ...
                .adjustsSurface()
                .register();
        RegistryKey<ConfiguredStructureFeature<?, ?>> cskeleton_configured = RegistryKey.of(Registry.CONFIGURED_STRUCTURE_FEATURE_KEY,
...
//           a RuntimeException can be thrown if a feature placer random grouping tries to place a feature in an unloaded chunk. i just ignore it
    ...
    public static BlockPos findNextFloor(StructureWorldAccess world, BlockPos start, int maxY, Tag<Block> floorType) {
        try {
            for (int i = 0; i < maxY; i++) {
                if (
                        !world.getBlockState(start.add(0, i, 0)).isOf(Blocks.POWDER_SNOW)
